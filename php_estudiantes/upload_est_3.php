...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
        ...
        $rutaDestino = "../uploads_est/uploads_est_3/" . $nombreArchivo;

        // Mueve el archivo a la carpeta de destino
        if (move_uploaded_file($rutaTemporal, $rutaDestino)) {
            // Guarda la información del archivo en la base de datos
            $consulta = $conexion->prepare("INSERT INTO tarea_3 (nombre_archivo, ruta_archivo) VALUES (?, ?)");
...
echo "<script>window.location.href = '../upload_estudiantes/upload_est_3.html';</script>";
?>
